...
dragons <- within(dragons, sample <- factor(mountainRange:site)) # create variable that is explicitely nested
...
# can use either (1|mountainRange/site) or even (1|mountainRange) + (1|mountainRange:site) but more advisable to ensure explicit naming within variables
...
# all of these have the same slope because we have only introduced random intercept models
...
# Plot predictions according to different levels of random effects
ggpredict(mixed.lmer2, terms = c("bodyLength2", "mountainRange"), type = "random") %>% # type argument updated from re to random
  plot() +
  labs(x = "Body Length", y = "Test Score", title = "Effect of body size on intelligence in dragons") + 
  theme_minimal()

# Visualize departures from overall model
library(sjPlot)
